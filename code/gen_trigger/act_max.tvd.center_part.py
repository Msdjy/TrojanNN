...
def make_step(net, xy, step_size=1.5, end='fc8', clip=True, unit=None, denoise_weight=0.1, margin=0, w=224, h=224):
    ...
    if end in fc_layers:
        ...
        if neuron_number == 1:
            xy_id = np.argmax([acts[0, unit1, xy, xy], acts[0, unit1, xy + 1, xy], acts[0, unit1, xy, xy + 1],
                               acts[0, unit1, xy + 1, xy + 1]])
            print(xy_id)
            if xy_id == 0:
            ...
            xy_id = np.argmax([acts[0, unit1, xy1, xy2], acts[0, unit1, xy1 + 1, xy2], acts[0, unit1, xy1, xy2 + 1],
                               acts[0, unit1, xy1 + 1, xy2 + 1]])
            print(xy_id)
            if xy_id == 0:
            ...
            xy_id = np.argmax([acts[0, unit2, xy1, xy2], acts[0, unit2, xy1 + 1, xy2], acts[0, unit2, xy1, xy2 + 1],
                               acts[0, unit2, xy1 + 1, xy2 + 1]])
            print(xy_id)
            if xy_id == 0:
    ...
    if end in fc_layers:
        ...
        obj_acts = [acts[0, unit, xy, xy], acts[0, unit, xy + 1, xy], acts[0, unit, xy, xy + 1],
                    acts[0, unit, xy + 1, xy + 1]]
...
def max_activation(net, layer, xy, base_img, octaves, random_crop=True, debug=True, unit=None,
    ...
    for e, o in enumerate(octaves):
        ...
        for i in xrange(o['iter_n']):
            ...
            denoise_weight = o['start_denoise_weight'] - (o['start_denoise_weight'] - (o['end_denoise_weight']) * i) / \
                             o['iter_n']

            best_unit, best_act, obj_act = make_step(net, xy, end=layer, clip=clip, unit=unit,
                                                     step_size=step_size, denoise_weight=denoise_weight,
                                                     margin=o['margin'], w=w, h=h)
